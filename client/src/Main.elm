...
type alias CurrentTurn = Stone

type alias Board = List (List (Maybe Stone))
...
playStone stone (xpos, ypos) board =
    let
        placeStone currentPoint =
            if (isStoneLegal stone currentPoint) then Just stone else currentPoint
        newRow row =
            List.indexedMap (\pointIndex currPoint -> if pointIndex == xpos then (placeStone currPoint) else currPoint) row
        newBoard =
            List.indexedMap (\rowIndex currRow -> if rowIndex == ypos then (newRow currRow) else currRow) board
    in
        newBoard
    ...
    case msg of
        Move coords ->
            case model.currentTurn of
                Black ->
                    { model | board = playStone Black coords model.board
                    , currentTurn = White }
                White ->
                    { model | board = playStone White coords model.board
                    , currentTurn = Black }
        default -> model
